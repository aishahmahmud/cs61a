...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if formals== nil:
            return child
        if Pair.__len__(formals) != Pair.__len__(vals):
            raise SchemeError
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        name= target.first
        variable= target.second
        body= expressions.second
        lambda_insides= Pair(variable, body)
        our_lambda= do_lambda_form(lambda_insides, env)
        env.define(name, our_lambda)
        return name
...
def do_or_form(expressions, env):
    """Evaluate a (short-circuited) or form . returns the FIRST TRUE"""
    # BEGIN PROBLEM 13
    if expressions==nil:
        return False
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        check_form(bind, 2, 2)
        x= bind.first
        v= scheme_eval(bind.second.first, env)
        forms= Pair(x, forms)
        vals= Pair(v, vals)
...
class MuProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 12
        return env.make_child_frame(self.formals,args)    # END PROBLEM 16
